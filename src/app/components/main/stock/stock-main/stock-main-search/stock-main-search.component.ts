...
export class StockMainSearchComponent implements OnInit {
  ...
  onSearch(event?: TablePageEvent) {
     ...
     this.criteria.name = this.searchKeyword;
  ...
  exportExcel() {
    ...
    const workbook = { Sheets: { Stock: worksheet }, SheetNames: ['Stock'] };
    const excelBuffer = XLSX.write(workbook, { bookType: 'xlsx', type: 'array' });
    const data = new Blob([excelBuffer], {
      type: 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet;charset=UTF-8'
    });
    const today = new Date();
    saveAs(data, `Stock_List_${today.getFullYear()}-${today.getMonth() + 1}-${today.getDate()}.xlsx`);
  }

  onclear() {
    this.searchKeyword = '';
    this.criteria = { name: "", sku: "", code: "" };
    this.onSearch();
  }
  ...
  onOpenEdit(id: number) { this.visibleEdit = true; }
  onDelete(id: number) { this.visibleDelete = true; }
  onCloseDelete() { this.visibleDelete = false; }
  onAdd() { this.router.navigate(['/stock-main-manage']); }
